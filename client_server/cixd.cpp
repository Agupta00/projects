...
void reply_rm (accepted_socket& client_sock, cix_header& header) {
   ...
   if (rc == -1) { 
      outlog << "rm failed: " << filename<< strerror (errno) << endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      send_packet (client_sock, &header, sizeof header);
      return;
...
void reply_get (accepted_socket& client_sock, cix_header& header) {
   ...
   if(!is.good()){
      outlog << "get failed: " << filename<< strerror (errno) << endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      send_packet (client_sock, &header, sizeof header);
      return;
...
void reply_put (accepted_socket& client_sock, cix_header& header) {
   ...
   ofstream outfile (header.filename, std::ofstream::binary);
   if(header.nbytes!=0){
   ...
   else    header.command = cix_command::ACK;

   outfile.close();

   header.nbytes = 0;
   outlog << " header " << header << endl;
   send_packet (client_sock, &header, sizeof header);
