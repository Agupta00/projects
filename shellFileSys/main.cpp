...
//initiates the root inode and inode state
void init(){
...
int main (int argc, char** argv) {
   ...
   //create root inode
   // inode rootNode(DIRECTORY_TYPE);
   // directory rootDirectory{};
   // rootNode.contents=make_shared<directory> (rootDirectory);

   // //init with root state
   // state.root = make_shared<inode>(rootNode);
   // state.cwd = make_shared<inode>(rootNode);
   ...
   try {
      for (;;) {
         try {
            ...
            const string command = words.at(0);
            if(command[0]=='#') continue;
            command_fn fn = find_command_fn (command);
